var tokenRequest = function() {
    ...
    let input = jQuery('.payment-cart-form').find('input');
    var error = false;

	jQuery('#card-number-error').html('');
	if (!luhnCheck(jQuery('.wc-credit-card-form-card-number').val())) {
		jQuery('#card-number-error').html('Wrong card number');
		return false;
	}

	input.each(function(){

    	if(jQuery(this).hasClass('notvalidfield')){
			error = true;
    	}
    	if(jQuery(this).val() == ''){
	        jQuery(this).addClass('notvalidfield');
			error = true;
    	}else{
	        jQuery(this).removeClass('notvalidfield');
    	}
    });
    if(error) {
...
/***************************/
/* global wc_checkout_params  checkout.js */
jQuery( function( $ ) {
	...
	var wc_checkout_form = {
		...
		submit: function() {
			...
			if ( $form.triggerHandler( 'checkout_place_order' ) !== false && $form.triggerHandler( 'checkout_place_order_' + wc_checkout_form.get_payment_method() ) !== false ) {
				...
				var browserInfo = '&browserInfo[deviceIdentity]='+identity+'&browserInfo[deviceTimeZone]='+timezone+'&browserInfo[deviceCapabilities]='+'javascript' + (java ? ',java' : '')+
					'&browserInfo[deviceAcceptLanguage]='+language+'&browserInfo[deviceScreenResolution]='+screen_width + 'x' + screen_height + 'x' + screen_depth;

				$.ajax({
					type:		'POST',
					url:		wc_checkout_params.checkout_url,
					data:		$form.serialize() + browserInfo,
					dataType:   'json',
					success:	function( result ) {
						...
						try {
							if ( 'success' === result.result && $form.triggerHandler( 'checkout_place_order_success' ) !== false ) {
								if(result.request) {
									window.location.href = result.request.processor;
								} else {
